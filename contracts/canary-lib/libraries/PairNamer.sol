...
// produces names for pairs of tokens using Canary's naming scheme
library PairNamer {
    string private constant TOKEN_SYMBOL_PREFIX = 'CNR:';
    ...
    // produces a pair symbol in the format of `YTS:${symbol0}:${symbol1}${suffix}`
    function pairSymbol(
        address token0,
        address token1,
        string memory suffix
